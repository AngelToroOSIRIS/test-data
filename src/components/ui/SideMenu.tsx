    ...
    return (
        <>
            <motion.div
                ...
                className={cn("w-full bg-background shadow-xl text-default-foreground fixed top-0 p-4 h-screen z-50", className, {
                    "left-0": position == "left",
                    "right-0 pl-10": position == "right",
                })}>
                {!disabledClosed && (
                    <i
                        className={cn("bi bi-x absolute text-borders top-0 hover:text-primary text-3xl transition-all cursor-pointer", {
            ...
            {background && (
                <motion.div
                    ...
                    className={cn("fixed bg-custom-black bg-opacity-70 top-0 right-0 z-40 w-full h-screen", {"bg-opacity-0": !backdrop})}
                    onClick={() => {
                        if (!disabledClosed) {
                            setShowAside(false)
                        }
